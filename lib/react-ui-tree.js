...
export default class UITree extends Component {
  ...
  dragEnd = () => {
    ...
    const node = index ? index.node : null;

    let parent = null
    if (index && index.parent) {
      const parentIdx = index.parent;
      const parentIndex = this.state.tree.getIndex(parentIdx);
      parent = parentIndex.node;
    }
    ...
    this.change(this.state.tree, node, parent);
  ...
  toggleCollapse = nodeId => {
    ...
    this.change(tree, node, index.parent);
  };
}
